...
int main()
{
    ...
    // ## Input validation test - try adding an edge to an NULL / non-existent Adjacency Matrix
    ...
    // ## Input validation test - try adding an edge with an invalid destination index, e.g.,
    //    negative or exceeding the size of the graph
    ...
    // ## Test running a depth-first traversal on a graph
    // This uses a ready-made graph. See 'Graph Example 2' in the Appendices
    // of the Assignment Description.
    printf("\nTEST: Running Depth-first traversal on Example Graph 2 with a starting node of 0:\n");
    fillUpMatrixWithDijkstraExampleGraph2(&adjacencyMatrix); 
    int traversalOutput[NUMBER_OF_VERTICES];
    for (int i=0; i<NUMBER_OF_VERTICES; i++)
        traversalOutput[i] = -1;
    doDepthFirstTraversal(&adjacencyMatrix, 0, traversalOutput);
    printf("Checking depth-first traversal output is 0 -> 1 -> 3 -> 4 -> 5 -> 2: ");
    if ((traversalOutput[0] == 0) && (traversalOutput[1] == 1) && (traversalOutput[2] == 3) && (traversalOutput[3] == 4) && (traversalOutput[4] == 5) && (traversalOutput[5] == 2))
        printf("PASS\n\n"); 
    else
        printf("FAIL\n\n");
